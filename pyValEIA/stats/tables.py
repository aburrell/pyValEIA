...
def decision_table_sat(states, sats=None, sat_key='Sat', eia_type='eia',
    ...
    if sats is None:
        sats = np.unique(states[sat_key].values)

    # Cycle over each unique satellite
    for i, s in enumerate(sats):
        # Sum total HMFCs
        hit = sum(states['skill'][states[sat_key] == s] == 'H')
        falarm = sum(states['skill'][states[sat_key] == s] == 'F')
        miss = sum(states['skill'][states[sat_key] == s] == 'M')
        corneg = sum(states['skill'][states[sat_key] == s] == 'C')
...
def style_df_table(df_table):
    ...
    ------
    ValueError
        For more than two state options or more than two MultiIndex levels

    """
    # Assert there are two levels and the second level has two values
    if len(df_table.index.levels) != 2:
        raise ValueError(
            'unknown input, expecting a MultiIndex with two levels')

    if len(df_table.index.levels[1]) != 2:
        raise ValueError('unknown input, expecting a state with two options')

    # Get the number of instruments
    ninst = len(df_table.index.levels[0])

    # Build a list of cells
    cell_list = list()
    for i in range(ninst):
        cell_list.extend([['true ', 'false '], ['false ', 'true ']])
    ...
    styled_frame.set_table_styles([
        ...
        overwrite=False)
    cell_color = pd.DataFrame(cell_list, index=df_table.index,
...
def lss_table_sat(model1, model2, model1_name='Model1', model2_name='Model2',
    ...
    if sats is None:
        sats = np.unique(model1[sat_key].values)

    # Cycle through all the desired satellites
    for i, s in enumerate(sats):
        # Calculate the Liemohn skill scores
        lss1_m1, lss2_m1, lss3_m1, lss4_m1 = skill_score.liemohn_skill_score(
            model1['skill'][model1[sat_key] == s])
        lss1_m2, lss2_m2, lss3_m2, lss4_m2 = skill_score.liemohn_skill_score(
            model2['skill'][model2[sat_key] == s])
...
def style_lss_table(lss_df, sat_names=None):
    ...
    """
    # Assert there are two levels
    if len(lss_df.index.levels) != 2:
        raise ValueError(
            'unknown input, expecting a MultiIndex with two levels')
